...
function createProjectForm(proj, i) {
  ...
  div.innerHTML = `
    ...
    <label>Tags (comma):</label><input type="text" value="${Array.isArray(proj.tags) ? proj.tags.join(', ') : proj.tags || ''}" onchange="updateProject(${i}, 'tags', this.value)">
...
function updateProject(i, key, value) { 
    if (key === "tags") {
        window.portfolioData.projects[i][key] = value.split(",").map(tag => tag.trim());
    } else {
        window.portfolioData.projects[i][key] = value;
    }
    saveData();
